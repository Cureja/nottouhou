...
for (var k = 0; k < 3; k++) {
	...
	MASTER.addEvent(2000 + k * 500, (MASTER) => {
		let enemies = [
			new Enemy(MASTER, "cirno", -28, app.renderer.height + 50, 5).addEvent(0, (enemy) => {
				enemy.addEvent(0, createLinearMovement(enemy.handle.x, enemy.handle.y, off, 28, 1000));
				return REMOVE_EVENT;
			}).addEvent(1000, (enemy) => {
			...
			new Enemy(MASTER, "playerIdle", app.renderer.width + 28, app.renderer.height + 50, 5).addEvent(0, (enemy) => {
				enemy.addEvent(0, createLinearMovement(enemy.handle.x, enemy.handle.y, app.renderer.width - off, 28, 1000));
				return REMOVE_EVENT;
			}).addEvent(1000, (enemy) => {
				new BoundedProjectile(MASTER, "projectileKnifeIdle180", enemy.handle.x, enemy.handle.y, 1).addEvent(0, (self) => {
					self.addEvent(0, createLinearProjection(self.handle.x, self.handle.y, player.handle.x, player.handle.y, 1000));
