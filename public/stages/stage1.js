		...
		for(var i = 0; i < 50; i++) {
			if(k == 0){
				...
				new BoundedProjectile(enemyProjectiles, "orbLightRed", 0, 0, 1)
					.dependOn(dependsEnemyAlive(spiralEnemies._gc))
					.setRelativeTo(spiralEnemies, 0, 0)
					.addEvent(200 * i, createArcingMovement(spiralEnemies.handle.x + projectionOne, spiralEnemies.handle.y + projectionTwo + (4 * i), app.renderer.width / 2 + (6z * i), app.renderer.height + 50, 2500));
			}
